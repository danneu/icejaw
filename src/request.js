...
module.exports = function request (url, {ignore404 = false, redirect = 'follow'} = {}) {
  ...
  return Promise.try(() => {
    ...
    if (err instanceof NotOkError && err.status === 404 && ignore404) {
      // handle 404
      return console.log(`${chalk.yellow('ignoring')} ${chalk.red(404)} ${route}`)
    } else if (err instanceof RedirectError && redirect === 'ignore') {
      return console.log(`${chalk.yellow('ignoring')} ${chalk.grey(err.status)} ${route}`)
    } else {
      throw err
    }
  })
